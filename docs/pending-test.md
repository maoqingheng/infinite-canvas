...
- 远程提示词同步写入数据库前，会按每个新出现的标签将对应第一条提示词标记为热门。
- 后台提示词管理列表、筛选和新增/编辑弹窗展示“是否热门”，保存时继续使用 `isHot` 字段。
- 风格库页面顶部新增标题、风格分类数量说明和按名称搜索入口，整体样式对齐提示词库。
- 首页热门风格和热门提示词区域初始各加载 12 条，按三行横向滑动展示；标题右侧“查看更多”跳转到对应库，横向列表末尾“查看更多热门”再加载该区域全部热门数据继续滑动查看。
- 提示词新增热门状态字段，后台提示词列表可按热门筛选，新增/编辑提示词时可设置普通或热门，公开提示词卡片和详情会显示热门标识。
- 提示词库列表卡片和详情弹窗新增“生成同款”入口，点击后会将提示词和封面图片模板带入生图工作台。
- 首页缩小“无限画布”首屏区域，新增热门风格展示区；首页通过 `ishot=1` 查询热门风格子项，点击卡片上的“生成同款”会像风格库一样带参考图和提示词跳转到生图工作台。
- 风格详情页点击“生成同款”跳转到生图工作台时，会把当前风格的提示词自动填入提示词框；没有提示词时仍使用原有默认文案。
